...
// NewAdapterWithDB is an alternative constructor for Adapter
// that does the same as NewAdapter, but uses existing mongo.Database provided
func NewAdapterWithDB(db *mongo.Database, dbName string) (persist.BatchAdapter, error) {
	a := &adapter{
		clientOption: nil,
	}
	a.filtered = false
